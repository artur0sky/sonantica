...
import { IconMusic, IconDisc, IconMicrophone, IconPlayerPlay } from "@tabler/icons-react";
              ...
              {trackRecommendations.map((rec) => (
                ...
                >
                  <div className="relative group">
                    <TrackItem
                      track={rec.item}
                      onClick={() => playNext({ ...rec.item, url: rec.item.path } as MediaSource)}
                    />
                    {/* Reason badge */}
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-3">
              {albumRecommendations.map((rec) => (
                ...
                <div key={rec.item.id} className="bg-surface-elevated/30 rounded-lg p-2 hover:bg-surface-elevated transition-colors cursor-pointer group">
                  <div className="aspect-square rounded-md overflow-hidden bg-surface-elevated mb-2 relative">
                    {rec.item.coverArt ? (
                      <img src={rec.item.coverArt} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <h4 className="text-xs font-medium truncate">{rec.item.name}</h4>
                  <p className="text-[10px] text-text-muted truncate">{rec.item.artist}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mb-8">
             ...
             <div className="space-y-2">
               {artistRecommendations.map((rec) => (
                 <div key={rec.item.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-surface-elevated transition-colors cursor-pointer">
                    ...
                    <div className="flex-1 min-w-0">
                       <h4 className="text-xs font-medium truncate">{rec.item.name}</h4>
                       <p className="text-[10px] text-text-muted">{rec.item.albums.length} albums</p>
                    </div>
