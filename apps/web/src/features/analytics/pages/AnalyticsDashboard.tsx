...
import { RealtimeTicker } from "@sonantica/ui";
...
export const AnalyticsDashboard = () => {
  ...
  const { data: realtimeData, loading: realtimeLoading } = useRealtimeStats();
  ...
  const stats = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: formatDuration(data.overview.totalPlayTime),
        unit: "hrs",
        trend: data.overview.playTimeChange,
        icon: <IconClock size={18} />,
      },
      {
        label: "Total Tracks",
        value: (
          data.overview.totalTracksInLibrary || data.overview.uniqueTracks
        ).toLocaleString(),
        unit: "tracks",
        trend: 0,
      ...
      },
      {
        label: "Total Artists",
        value: (
          data.overview.totalArtistsInLibrary || data.overview.uniqueArtists
        ).toLocaleString(),
        unit: "artists",
        trend: 0,
      ...
      },
      {
        label: "Total Albums",
        value: (
          data.overview.totalAlbumsInLibrary || data.overview.uniqueAlbums
        ).toLocaleString(),
        unit: "albums",
        trend: 0,
      ...
      {
        label: "Daily Streak",
        value: data.listeningStreak.currentStreak.toString(),
        unit: "days",
        trend: 0,
  ...
  const playbackData = useMemo(() => {
    if (!data?.playbackTimeline || data.playbackTimeline.length === 0)
      return [];
  ...
  const topTracks = useMemo(() => {
    ...
    const maxPlays =
      data.topTracks.length > 0 ? data.topTracks[0].playCount : 1;
  ...
  const formatData = useMemo(() => {
    if (!data?.overview.formats || data.overview.formats.length === 0)
      return [];
    ...
    return [
      {
        id: "Cover",
        label: "With Cover",
        value: data.overview.coverArtStats.withCover,
      },
      {
        id: "Missing",
        label: "Missing",
        value: data.overview.coverArtStats.withoutCover,
      },
    ];
  }, [data]);
  ...
  const platformData = useMemo(() => {
    ...
    return data.platformStats.map((p) => ({
      ...
      label:
        (p.platform || "unknown").charAt(0).toUpperCase() +
        (p.platform || "unknown").slice(1),
      value: p.sessionCount || 0,
    }));
  }, [data]);

  const heatmapData = useMemo(() => {
    if (!data?.listeningHeatmap || data.listeningHeatmap.length === 0)
      return [];
    ...
    return [
      {
        ...
        colSpan: 4 as const,
        component: (
          <RealtimeTicker data={realtimeData} loading={realtimeLoading} />
        ),
      },
      ...stats.map((s, i) => ({
        id: `stat-${i}`,
      ...
      {!data && loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-pulse">
          {[...Array(8)].map((_, i) => (
            <div
              key={i}
              className="h-32 bg-surface/40 rounded-2xl border border-border/50"
            />
          ))}
        </div>
      ) : (
